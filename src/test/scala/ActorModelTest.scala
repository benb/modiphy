...
import org.modiphy.sequence.BioEnum
...
class ActorModelSuite extends FunSuite {
  ...
  test ("Actor Model should give correct matrix exp"){
    case class Test[A <: BioEnum](b:Branch[A])
    class ActorTest extends Actor{
      ...
      val ans = (actor !? Test(tree.bList.head.myBranch)).asInstanceOf[MatReq[alphabet.type]]
      val mat1 = ans.m.get
      ...
      val b2 = b1.down.bList.filter{_.myBranch != b1.myBranch}.head
       ...
       val mat3 = (actor !? Test(b2.myBranch)).asInstanceOf[MatReq[alphabet.type]].m.get
